...
#define SAMPLING_FREQ   40000         // Hz, must be 40000 or less due to ADC conversion time. Determines maximum  y that can be analysed by the FFT Fmax=sampleF/2.
#define AMPLITUDE       1000          // Depending on your audio source level, you may need to alter this value. Can be used as a 'sensitivity' control.
#define AUDIO_IN_PIN    35            // Signal in on this pin
#define LED_PIN         25             // LED strip data
...
void loop() {
  ...
  for (int i = 2; i < (SAMPLES/2); i++){       // Don't use sample 0 and only first SAMPLES/2 are usable. Each array element represents a frequency bin and its value the amplitude.
    ...
    /*8 bands, 12kHz top band
     ...
     //12 bands, 12kHz top band
      if (i<=3 )           bandValues[0]  += (int)vReal[i];
      if (i>3   && i<=4  ) bandValues[1]  += (int)vReal[i];
      if (i>4   && i<=7  ) bandValues[2]  += (int)vReal[i];
      if (i>7   && i<=10  ) bandValues[3]  += (int)vReal[i];
      if (i>10   && i<=16  ) bandValues[4]  += (int)vReal[i];
      if (i>16   && i<=26 ) bandValues[5]  += (int)vReal[i];
      if (i>26  && i<=41 ) bandValues[6]  += (int)vReal[i];
      if (i>41  && i<=64 ) bandValues[7]  += (int)vReal[i];
      if (i>64  && i<=101 ) bandValues[8]  += (int)vReal[i];
      if (i>101  && i<=159 ) bandValues[9]  += (int)vReal[i];
      if (i>159  && i<=251 ) bandValues[10] += (int)vReal[i];
      if (i>251  && i<=396 ) bandValues[11] += (int)vReal[i];
